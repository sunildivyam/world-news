...
export async function seedLanguages(): Promise<void> {
  ...
  const languages = db.collection<any>("languages");

  const seedDataPath = path.join(
    __dirname,
    "../../seed-data/seed_languages.json",
  );
  const data = fs.readFileSync(seedDataPath, "utf-8");
  const languagesP = JSON.parse(data);

  // Add your database insertion logic here
  console.log(`Seeding ${languagesP.length} languagesP...`);
  const seedResult = await languages.insertMany(languagesP);
  console.log("Seed Final: ", seedResult);
}

const result = await seedLanguages().catch((err) => {
